...
$(function() {
  ...
  $('#add-service-field')
    .search({
      apiSettings: {
        url: '/clinic/services?query={query}'
      },
      onSelect: function(result, response) {
        // console.log(result);
        // console.log(response);
        // $('.ui.search').search('hide results');
        window.location.href = '/clinic/services/' + result.service_id + '/attachDoctor?doctor_id={{ $doctor->id }}';
        return false;
      },
      cache: false,
      error: {
        noResults: '{{ trans('main.serviceIsNotFound') }}',
...
<form class="ui form attached segment">
  <div class="ui dropdown icon search input" id="add-service-field">
    <input type="text" class="prompt" placeholder="{{ trans('main.assignService') }}..." />
...
<table class="ui celled striped centered bottom attached table">
  ...
  <tbody>
    ...
    <tr>
      <td><i class="share alternate icon"></i> <a href="/clinic/services/{{ $service->id }}">{{ $service->name }}</a></td>
      <td class="center aligned collapsing">
        <div class="ui icon menu">
          <a class="red popup item" data-content="{{ trans('main.unassignService') }}" href="/clinic/services/{{ $service->id }}/detachDoctor?doctor_id={{ $doctor->id }}">
    ...
    @else
    <tr>
      <td colspan="2" class="center aligned">{{ trans('main.doctorIsNotNotAssociatedWithAnyService') }}</td>
    </tr>
    @endif
  </tbody>
</table>
